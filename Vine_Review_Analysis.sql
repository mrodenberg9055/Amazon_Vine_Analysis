...
SELECT COUNT(review_id) AS total_reviews
	, SUM(five_star_rating) AS five_star_rating
	, SUM(COALESCE(paid, 0)) AS total_paid
	, SUM(COALESCE(paid, 0)) / COUNT(review_id)::FLOAT AS paid_percent
	, SUM(COALESCE(unpaid, 0)) AS total_unpaid
	, SUM(COALESCE(unpaid, 0)) / COUNT(review_id)::FLOAT  AS unpaid_percent
FROM reviews
